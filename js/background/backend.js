...
class Backend {
    constructor() {
        ...
        const HEADERS_TO_APPEND = {
            'cache-control': 'public,max-age=31536000,immutable,max-age=604800,must-revalidate'
        };

        chrome.webRequest.onHeadersReceived.addListener(
            (details) => {
                const headersToRemove = [...HEADERS_TO_STRIP_LOWERCASE, ...Object.values(HEADERS_TO_APPEND)];
                let headers = details.responseHeaders.filter((header) => !headersToRemove.includes(header.name.toLowerCase()));

                for (const [header, value] of Object.entries(HEADERS_TO_APPEND)) {
                    headers.push({
                        name: header,
                        value
                    });
                ...
                if (details.url.includes('hanziyuan')) {
                    headers = headers.map((header) => {
                        if (header.name === 'Set-Cookie') {
                            header.value = header.value.replace('samesite=strict', 'samesite=none');
                            if (!header.value.includes(' secure;')) {
                                header.value += '; secure;';
                            }
                        }
                        return header;
                    ...
                    // console.log(details.initiator, headers);
                }
        ...
        const displayGenerator = new DisplayGenerator({
            japaneseUtil: new JapaneseUtil(null)
        });

        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            if (message.cmd === EXTENSION_COMMANDS.QUERY_YOMICHAN) {
                (async () => {
                    const words = message.words ? message.words : [];
                    const results = [];
                    for (const word of words) {
                        const result = await this._onApiTermsFind({
                            text: word,
                            details: {},
                            optionsContext: {}
                        });

                        const dictionaryEntry = result.dictionaryEntries[0];

                        const {headwords, pronunciations} = dictionaryEntry;
                        for (let i = 0; i < headwords.length; i++) {
                            const {reading, wordClasses} = headwords[i];

                            /* eslint-disable */
                            const pronunciationCategories = displayGenerator._getPronunciationCategories(reading, pronunciations, wordClasses, i);

                            results.push(pronunciationCategories);
                        }
                    }
                ...
                return true;
            } else if (message.cmd === EXTENSION_COMMANDS.PITCH_COLOR) {
                chrome.tabs.sendMessage(sender.tab.id, {
                    cmd: EXTENSION_COMMANDS.PITCH_COLOR
                })
            } else if (message.cmd === EXTENSION_COMMANDS.OPEN_NEW_TAB) {
                chrome.tabs.create({ url: message.url });
            } else if (message.cmd === EXTENSION_COMMANDS.TOGGLE_FURIGANA) {
                chrome.tabs.sendMessage(sender.tab.id, {
                    cmd: EXTENSION_COMMANDS.TOGGLE_FURIGANA,
                    visible: message.visible,
                });
            } else if (message.cmd === EXTENSION_COMMANDS.TOGGLE_FURIGANA_JPDB) {
                chrome.tabs.sendMessage(sender.tab.id, {
                    cmd: EXTENSION_COMMANDS.TOGGLE_FURIGANA_JPDB,
                    visible: message.visible,
                });
            }
